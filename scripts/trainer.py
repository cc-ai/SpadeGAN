...
from networks import SpadeGen, MsImageDis, VAEGen, MultiscaleDiscriminator, VGGLoss, SpadeAdaINGenII
from utils import (
    weights_init,
    get_model_list,
    vgg_preprocess,
...
class MUNIT_Trainer(nn.Module):
    ...
    def sample(self, x_a, x_b, m_a, m_b, rect_b=None):
        ...
        for i in range(x_a.size(0)):
            ...
            x_ab1.append(
                self.gen(x_a[i].unsqueeze(0), m_a[i].unsqueeze(0), x_b[i].unsqueeze(0))
            )  # s_b1[i].unsqueeze(0)))
            x_ab2.append(
                self.gen(x_a[i].unsqueeze(0), m_a[i].unsqueeze(0), x_b[i].unsqueeze(0))
            )  # s_b1[i].unsqueeze(0)))
